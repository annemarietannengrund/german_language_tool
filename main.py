...
class GermanLanguageTool(tk.Tk):
    ...
    translation_file = "assets/translated.json"
    translate_map: dict
    frameregister = {}
    ...
    def load_translations(self, lang):
        ...
        with open(path, "r", encoding="utf-8") as f:
            ...
            self.translations = filecontent.get("translation_base")
            self.replace_words = filecontent.get("reserved")
        if not self.available_languages:
            for code, content in self.translations.items():
        ...
        self.translate_raw_map = self.translations.get(lang)
        self.init_string_var_translation_components()

    def update_translation(self, key, new_text):
        self.translate_map[key].set(new_text)

    def replace_reserved_words(self, translation):
        for key, replacement in self.replace_words.items():
